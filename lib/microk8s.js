...
class MicroK8s {
    constructor(channel, addons, devMode, launchConfigPath, sideloadImagePath, dockerUser, dockerPassword) {
    ...
    dockerLogin() {
        sh.echo('test 1');
        if (this.dockerUser !== "") {
            sh.echo('dockerUser supplied');
        }
        if (this.dockerPassword !== "") {
            sh.echo('dockerPassword supplied');
        }
        if (this.dockerUser !== "" && this.dockerPassword !== "") {
            sh.echo('logging in to docker.io with supplied user and password');
